...
func (suite *Suite) TestScheduler_ExpectTaskExecutedAndUpdated() {
	...
	task := suite.GivenTask(1, executeDate, 10*time.Minute)

	suite.RunServerSchedulerUntilTaskCounterIncreased(task)
...
func (suite *Suite) TestScheduler_ExpectTaskDidNotExecuteAndUpdated() {
	...
	task := suite.GivenTask(2, executeDate, 10*time.Minute)
